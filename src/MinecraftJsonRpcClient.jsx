...
const SETTINGS = [
  ...
  { key: "difficulty", type: "enum", options: ["peaceful", "easy", "normal", "hard"] },
  ...
  { key: "game_mode", type: "enum", options: ["survival", "creative", "adventure", "spectator"] },
...
export default function MinecraftJsonRpcClient() {
  ...
  const connect = () => {
    ...
    try {
      ...
      wsRef.current = ws
      ws.onopen = () => { setIsOpen(true); log("[open]", { url }) }
      ws.onmessage = ev => {
        let data = ev.data
        try { data = JSON.parse(ev.data) } catch {}
        log("[recv]", data)
      }
      ws.onclose = ev => { setIsOpen(false); log("[close]", { code: ev.code, reason: ev.reason }) }
      ws.onerror = ev => { log("[error]", { message: "WebSocket error", ev: String(ev) }) }
    } catch (e) {
  ...
  const addOp = () => {
    if (!opName) return
    sendRpc("minecraft:operators/add", [[{
      player: { name: opName },
      permissionLevel: Number(opLevel),
      bypassesPlayerLimit: Boolean(opBypass)
    }]])
  }
  const removeOp = () => { if (!opName) return; sendRpc("minecraft:operators/remove", [[{ name: opName }]]) }
  ...
  const addBans = () => {
    const players = splitList(banNames)
  ...
  const removeBans = () => {
    const players = splitList(banNames).map(name => ({ name }))
    if (!players.length) return
  ...
  const sendRaw = () => {
    try {
      ...
      log("[error]", { message: "Invalid JSON in Raw sender", error: String(e) })
    }
  }

  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold">Minecraft JSON-RPC WebSocket Client</h1>

      {/* Connection Bar */}
      <div className="grid grid-cols-1 md:grid-cols-5 gap-3 items-center p-4 rounded-2xl bg-white border shadow-sm">
        <input className="md:col-span-3 rounded-xl border px-3 py-2" placeholder="wss://your-endpoint" value={url} onChange={(e)=>setUrl(e.target.value)} />
        <div className="md:col-span-2 flex gap-2 justify-end">
          {!isOpen ? (
          ...
          <button className="px-4 py-2 rounded-xl bg-blue-600 text-white" onClick={status}>Server Status</button>
